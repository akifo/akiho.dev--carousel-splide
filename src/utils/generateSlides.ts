...
export function generateSlidesFromSwiperJs(
  length = 10,
  sig = 0,
  type = "nature", // "movies" | "nature" | "model"
): Array<{ src: string; alt: string }> {
  return Array.from({ length }).map((value, index) => {
    index = (sig || index) + 1;
    const stringNumber = index.toString().padStart(2, "0");

    return {
      src: `https://studio.swiperjs.com/demo-images/${type}/${stringNumber}.jpg`,
      alt: `Image ${stringNumber}`,
    };
  });
}
